...
class DQN(object):
    ...
    def __init__(self, n_actions, hidden=1024, learning_rate=0.00001, screen_height=84, screen_width=84, number_of_frames=4):
        ...
        self.input = tf.placeholder(shape=[None, self.screen_height, 
                                           self.screen_width, self.number_of_frames], 
                                    dtype=tf.float32)
        # transformam intensitatile de culoare intre valori cuprinse intre 0 si 1
        self.inputscaled = self.input/255
        
        # straturile convulationale
        self.conv1 = tf.layers.conv2d(inputs=self.inputscaled, filters=32, kernel_size=[8, 8], strides=4,kernel_initializer=tf.variance_scaling_initializer(scale=2),padding="valid", activation=tf.nn.leaky_relu, use_bias=False, name='conv1')
        self.conv2 = tf.layers.conv2d(inputs=self.conv1, filters=64, kernel_size=[4, 4], strides=2, kernel_initializer=tf.variance_scaling_initializer(scale=2),padding="valid", activation=tf.nn.leaky_relu, use_bias=False, name='conv2')
        self.conv3 = tf.layers.conv2d(inputs=self.conv2, filters=64, kernel_size=[3, 3], strides=1, kernel_initializer=tf.variance_scaling_initializer(scale=2),padding="valid", activation=tf.nn.leaky_relu, use_bias=False, name='conv3')
        
        # impartim iesirea ultimului strat convulational intre fluxul de valoare si cel de avantaj
        self.flat = tf.layers.flatten(self.conv3)
        self.valuestream = tf.layers.dense(inputs=self.flat, units=512, activation=tf.nn.leaky_relu,name='valuestream', kernel_initializer=tf.variance_scaling_initializer(scale=2))
        self.advantagestream = tf.layers.dense(inputs=self.flat, units=512, activation=tf.nn.leaky_relu, name='advantagestream',kernel_initializer=tf.variance_scaling_initializer(scale=2))
