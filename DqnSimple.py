...
class DQN(object):
    ...
    def __init__(self, n_actions, hidden=1024, learning_rate=0.00001, screen_height=84, screen_width=84, number_of_frames=4):
        ...
        self.conv1 = tf.layers.conv2d(inputs=self.inputscaled, filters=8, kernel_size=[20, 20], strides=4,kernel_initializer=tf.variance_scaling_initializer(scale=2),padding="valid", activation=tf.nn.relu, use_bias=False, name='conv1')
        self.conv2 = tf.layers.conv2d(inputs=self.conv1, filters=32, kernel_size=[4, 4], strides=2, kernel_initializer=tf.variance_scaling_initializer(scale=2),padding="valid", activation=tf.nn.relu, use_bias=False, name='conv2')
        ...
        self.valuestream = tf.layers.dense(inputs=self.flat, units=512, activation='relu',name='valuestream', kernel_initializer=tf.variance_scaling_initializer(scale=2))
        self.advantagestream = tf.layers.dense(inputs=self.flat, units=512, activation='relu', name='advantagestream',kernel_initializer=tf.variance_scaling_initializer(scale=2))
